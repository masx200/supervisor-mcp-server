...
export class LogReader {
   ...
   * 从文件指定位置读取日志内容（支持分页）
   */
  static async readLog(logFilePath: string, options: LogReadOptions = {}): Promise<LogContent> {
    ...
    try {
      ...
      throw new Error(`Failed to read log file '${logFilePath}': ${error.message}`);
    }
  }
   ...
   * 按字节长度读取文件内容
   */
  private static readByLength(filePath: string, offset: number, length: number): string {
   ...
   * 从文件末尾读取指定行数
   */
  private static async readFromTail(filePath: string, lines: number): Promise<string> {
    return new Promise((resolve, reject) => {
      const fileStream = createReadStream(filePath, { start: 0, end: undefined });
   ...
   * 读取文件的前 N 行
   */
  static async readFirstLines(filePath: string, lineCount: number): Promise<string> {
    return new Promise((resolve, reject) => {
      const fileStream = createReadStream(filePath, { start: 0, end: undefined });
      const readline = createInterface({ input: fileStream });
      const lines: string[] = [];
   ...
   * 读取文件的后 N 行
   */
  static async readLastLines(filePath: string, lineCount: number): Promise<string> {
